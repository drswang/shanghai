"""
2×2 map overview of proc1 (ctrl) at 4 timesteps:
  b: t49  — Predecessor Rain #1  (Aug 21 00:00)
  c: t90  — Predecessor Rain #2  (Aug 22 17:00)
  d: t275 — Before Typhoon Impact (Aug 30 10:00)
  e: t357 — After Typhoon Impact  (Sep 02 20:00)
"""
...
CTRL_DIR = f"{BASE}/postproc/new_postproc1/infil_7_890_rain/text"
...
def shuai_colormap1():
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list("shuai1", np.concatenate([tmp1, tmp2, tmp4]))
...
show_x = [False, False, True, True]
show_y = [True,  False, True, False]

for ax, t, label, title, sx, sy in zip(axes, TIMESTEPS, PANEL_LABELS, MAP_TITLES, show_x, show_y):
    print(f"  {label}: t{t} — {title.replace(chr(10), ' ')}…")
    gdf = load_xyz(f"{CTRL_DIR}/t{t}.xyz")
    ...
    ax.set_xticklabels(x_labels if sx else [], fontsize=7)
    ax.set_yticklabels(y_labels if sy else [], fontsize=7)
    if sx:
...
# ── Colorbar ──────────────────────────────────────────────────────────────
cbar_ax = fig.add_axes([0.92, axes[2].get_position().y0, 0.015,
                         axes[0].get_position().y1 - axes[2].get_position().y0])
...
out = f"{BASE}/Fig_Overview_maps_new.png"
